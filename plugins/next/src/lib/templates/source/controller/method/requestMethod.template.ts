...
export function nextRequestMethodTemplate(
  ...
  internalGeneratorContext.getCounter(source)?.inc("Server Side Async Methods")
  ...
  imports.add(
    `import {getAxiosInstance, addResponseToHydrate, getHeaders} from './intrig-middleware'`,
  );
  imports.add(
    `import {transformResponse, encode} from './media-type-utils'`,
  );
  imports.add(
    `import { networkError, responseValidationError, AsyncRequestOptions } from './network-state';`,
  );
  ...
  if (requestBody) {
    imports.add(
      `import { ${requestBody} as RequestBody, ${requestBody}Schema as requestBodySchema } from "./${source}/components/schemas/${requestBody}"`,
    );
  }

  if (response) {
    imports.add(
      `import { ${response} as Response, ${response}Schema as schema } from "./${source}/components/schemas/${response}"`,
    );
  }
  ...
  errorTypes.forEach((ref) =>
    imports.add(
      `import {${ref}, ${ref}Schema } from "./${source}/components/schemas/${ref}"`,
    ),
  );
