...
Next.js uses a simplified single configuration approach where:
- Server-side: API source URLs managed through environment variables
- Client-side: Single configuration object via IntrigLayout (not per-source configs)
